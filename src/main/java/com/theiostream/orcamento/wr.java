	...
	public static void main (String[] args) {
		...
		try {
			...
			String construct = StrUtils.strjoinNL(
				...
				"} where {",
				"	{ select (sample(?s) as ?clown) ?po ?ma ?iu ?fn ?ac ?ex ?fr ?ce ?es ?sf ?st ?pr ?ed ?uo ?gnd (sum(?di) as ?tdi) (sum(?pg) as ?tpg) (sum(?pl) as ?tpl) where {",
				...
				"			rdf:type loa:ItemDespesa",
				"		}",
				"	group by ?po ?ma ?iu ?fn ?ac ?ex ?fr ?ce ?es ?sf ?st ?pr ?uo ?gnd ?ed }",
				"}");
